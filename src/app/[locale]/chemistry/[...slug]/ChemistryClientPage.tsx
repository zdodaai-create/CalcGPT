...
export default function ChemistryClientPage({ slugArray }: { slugArray: string[] }) {
  const tCalc = useTranslations('Calculators');

  const { category, calculator, type } = useMemo(() => {
    // Case 1: /chemistry/category-id
    if (slugArray.length === 1) {
      const cat = CHEMISTRY_DATA.find(c => c.id === slugArray[0]);
      if (cat) return { category: cat, calculator: null, type: 'category' };
      
      // Case 2: /chemistry/calculator-slug (searching all categories)
      for (const c of CHEMISTRY_DATA) {
        const calc = c.calculators.find(cal => cal.slug === slugArray[0]);
        if (calc) return { category: c, calculator: calc, type: 'calculator' };
      }
    }
    
    // Case 3: /chemistry/category-id/calculator-slug
  ...
  if (type === '404') {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center p-4">
        ...
        <p className="text-gray-500 mb-8">We couldn't find the chemistry calculator or category you're looking for.</p>
        <Link href="/chemistry" className="bg-blue-600 text-white px-8 py-3 rounded-full font-bold">
          Back to Chemistry
        </Link>
      </div>
    );
      ...
      <div className="pt-12 pb-20">
        <div className="max-w-7xl mx-auto px-4">
          <nav className="flex items-center gap-2 text-sm text-gray-500 mb-10">
            ...
            <Link href="/chemistry" className="hover:text-blue-600">Chemistry</Link>
            <ChevronRight className="w-4 h-4" />
          ...
          <div className="mb-12">
             ...
             <p className="text-xl text-gray-500 max-w-2xl font-medium">
                Comprehensive collection of specialized high-fidelity tools for {category?.title.toLowerCase()}.
             </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {category?.calculators.map((calc) => (
              <motion.div key={calc.slug} whileHover={{ y: -5 }}>
                <Link href={`/chemistry/${category.id}/${calc.slug}`}>
    ...
    <div className="pt-8 pb-20">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <nav className="flex items-center gap-2 text-sm text-gray-500 mb-8">
          ...
          {category && (
            <>
              <Link href={`/chemistry/${category.id}`} className="hover:text-blue-600">{category.title}</Link>
              <ChevronRight className="w-4 h-4" />
            </>
          )}
          ...
          <div className="lg:col-span-8">
            <div className="mb-8">
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm font-bold">
                <span className="px-4 py-1.5 bg-blue-50 text-blue-600 rounded-full">Chemistry</span>
               ...
               {(() => {
                 ...
                 return (
                    <div className="text-center py-12">
                       ...
                       <p className="text-gray-500 mb-8 max-w-md mx-auto">This chemical tool is currently being optimized. You can use our AI Generator to build it instantly.</p>
            ...
            <article className="prose prose-lg max-w-none prose-headings:font-black prose-headings:text-gray-900 prose-p:text-gray-600 prose-p:leading-relaxed">
              ...
              <p>
                The {calculator?.name} is a highly precise scientific tool designed for chemical research, laboratory calculations, and educational analysis. 
                Whether you are a researcher in a lab, a university student, or an industry professional, this calculator helps you achieve 
                accurate results with rigorous standards.
              </p>
              <h3>Scientific Application & Accuracy</h3>
              <p>
                Simply input your measured parameters into the input fields above. Our engine uses standard chemical equations, constant values, and strict IUPAC conventions to process your values and provide immediate scientific answers.
              </p>
              <div className="bg-blue-50 p-8 rounded-3xl border border-blue-100 not-prose my-12">
                 ...
                 <p className="text-blue-800 font-medium">
                    Ensure the correct unit scales are chosen. Stoichiometric calculations depend on correct molecular quantities, standard pressures (atm/bar), and absolute temperatures (Kelvin).
                 </p>
              </div>
            </article>
          </div>
          ...
          <aside className="lg:col-span-4 space-y-10">
              ...
              <div className="space-y-4">
                {category?.calculators
                  ...
                  .map(related => (
                    <Link 
                      key={related.slug} 
                      href={`/chemistry/${category.id}/${related.slug}`}
                      className="flex items-center justify-between p-4 rounded-2xl hover:bg-blue-50 transition-colors group"
                    >
                      <span className="font-bold text-gray-700 group-hover:text-blue-600 transition-colors text-sm leading-snug">{related.name}</span>
                      <ChevronRight className="w-4 h-4 text-gray-300 group-hover:text-blue-600 shrink-0 ml-2" />
                    </Link>
                  ))}
              </div>
            </div>
            ...
            <div className="bg-[#111] p-8 rounded-[2rem] text-white overflow-hidden relative">
               <div className="relative z-10">
                 ...
                 <p className="text-gray-400 mb-8 font-medium">Download our mobile application for offline access to all chemical tools.</p>
