...
export default function StatisticsMegaCategoryPage() {
  ...
  const filteredData = useMemo(() => {
    ...
    if (!query) return STATISTICS_DATA;
    ...
    <div className="min-h-screen bg-[#F8F9FA] pt-12 pb-20">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <nav className="flex items-center gap-2 text-sm text-gray-500 mb-10">
          ...
          <span className="text-gray-900 font-bold">Statistics</span>
        </nav>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
            <div className="max-w-2xl">
              <div className="flex items-center gap-4 mb-6">
                <div className="w-14 h-14 rounded-2xl bg-slate-600 flex items-center justify-center shadow-xl shadow-slate-500/20">
              ...
              <p className="text-xl text-gray-500 font-medium leading-relaxed">
                Explore descriptive statistics, probability, hypothesis testing, regression, confidence intervals, and advanced data science metrics.
              </p>
            </div>
            <div className="hidden lg:block">
               <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-200 max-w-sm">
                  <h3 className="font-bold text-slate-900 mb-2">Need a custom statistical tool?</h3>
                  <p className="text-slate-700/80 text-sm mb-4">Our AI can build any statistical analysis calculator in seconds.</p>
                  <Link href="/ai-generator" className="inline-flex items-center gap-2 text-slate-600 font-bold text-sm hover:gap-3 transition-all">
            ...
            <div key={category.id} className="scroll-mt-32">
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="p-3 bg-white rounded-2xl shadow-sm border border-gray-100 text-slate-600 group-hover:scale-110 transition-transform">
                    <category.icon className="w-6 h-6" />
                  </div>
                  <div>
                    <h2 className="text-2xl font-extrabold text-gray-900 group-hover:text-slate-600 transition-colors flex items-center gap-3">
                      {category.title}
                      <span className="text-sm font-black text-gray-400 bg-gray-100 px-3 py-1 rounded-full group-hover:bg-slate-100 group-hover:text-slate-600 transition-colors">
        ...
        <section className="mt-24 pt-16 border-t border-gray-100">
           <h2 className="text-3xl font-black text-gray-900 mb-8">Why use our Statistics Calculators?</h2>
           <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                {
                  title: "Comprehensive Analysis",
                  desc: "From basic means and standard deviations to complex multiple linear regression models and hypothesis tests."
                },
                {
                  title: "Data Science Metrics",
                  desc: "Evaluate classifier performance with confusion matrices, AUC, precision, recall, and F1 scores."
                },
                {
                  title: "Accurate Probabilities",
                  desc: "Compute exact probability distributions including Normal, Student's t, Chi-Square, and F-distributions."
                }
              ].map((item, i) => (
