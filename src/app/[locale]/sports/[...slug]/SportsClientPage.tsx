...
export default function SportsClientPage({ slugArray }: { slugArray: string[] }) {
  const tCalc = useTranslations('Calculators');

  const { category, calculator, type } = useMemo(() => {
    // Case 1: /sports/category-id
    if (slugArray.length === 1) {
      const cat = SPORTS_DATA.find(c => c.id === slugArray[0]);
      if (cat) return { category: cat, calculator: null, type: 'category' };
      
      // Case 2: /sports/calculator-slug (searching all categories)
      for (const c of SPORTS_DATA) {
        const calc = c.calculators.find(cal => cal.slug === slugArray[0]);
        if (calc) return { category: c, calculator: calc, type: 'calculator' };
      }
    }
    
    // Case 3: /sports/category-id/calculator-slug
  ...
  if (type === '404') {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center p-4">
        ...
        <p className="text-gray-500 mb-8">We couldn't find the sports calculator or category you're looking for.</p>
        <Link href="/sports" className="bg-emerald-600 text-white px-8 py-3 rounded-full font-bold">
          Back to Sports
        </Link>
      </div>
    );
      ...
      <div className="pt-12 pb-20">
        <div className="max-w-7xl mx-auto px-4">
          <nav className="flex items-center gap-2 text-sm text-gray-500 mb-10">
            ...
            <Link href="/sports" className="hover:text-emerald-600">Sports</Link>
            <ChevronRight className="w-4 h-4" />
            ...
            {category?.calculators.map((calc) => (
              <motion.div key={calc.slug} whileHover={{ y: -5 }}>
                <Link href={`/sports/${category.id}/${calc.slug}`}>
                  <Card className="h-full border-gray-100 hover:border-emerald-500/20 hover:shadow-xl transition-all rounded-[1.5rem]">
                    <CardContent className="p-8">
                      <div className="w-10 h-10 bg-gray-50 rounded-xl flex items-center justify-center mb-6 group-hover:bg-emerald-50 transition-colors">
                        <CalcIcon className="w-5 h-5 text-gray-400 group-hover:text-emerald-600" />
                      </div>
    ...
    <div className="pt-8 pb-20">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <nav className="flex items-center gap-2 text-sm text-gray-500 mb-8">
          ...
          {category && (
            <>
              <Link href={`/sports/${category.id}`} className="hover:text-emerald-600">{category.title}</Link>
              <ChevronRight className="w-4 h-4" />
            </>
          )}
          ...
          <div className="lg:col-span-8">
            <div className="mb-8">
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm font-bold">
                <span className="px-4 py-1.5 bg-emerald-50 text-emerald-600 rounded-full">Sports</span>
               ...
               {(() => {
                 ...
                 return (
                    <div className="text-center py-12">
                       ...
                       <p className="text-gray-500 mb-8 max-w-md mx-auto">This sports tool is currently being optimized. You can use our AI Generator to build it instantly.</p>
                       <Link href="/ai-generator" className="bg-emerald-600 text-white px-8 py-4 rounded-2xl font-bold hover:bg-emerald-700 transition-all shadow-xl shadow-emerald-500/20 inline-flex items-center gap-2">
            ...
            <article className="prose prose-lg max-w-none prose-headings:font-black prose-headings:text-gray-900 prose-p:text-gray-600 prose-p:leading-relaxed">
              ...
              <p>
                The {calculator?.name} is a specialized tool designed for athletic training, performance analysis, and sports statistics. 
                Whether you are a recreational athlete, coach, or sports fan, this calculator helps you compute accurate metrics and projections.
              </p>
              <h3>How it works</h3>
              <p>
                Input your values into the fields above. Our calculation engine computes the results using standard athletic equations and formulas instantly.
              </p>
              <div className="bg-emerald-50 p-8 rounded-3xl border border-emerald-100 not-prose my-12">
                 <h4 className="text-emerald-900 font-bold text-xl mb-4 flex items-center gap-2">
                    <Sparkles className="w-5 h-5" /> Pro Tip
                 </h4>
                 <p className="text-emerald-800 font-medium">
                    Consistently track your metrics over time to notice patterns and monitor fitness progression.
                 </p>
              </div>
            </article>
          </div>
          ...
          <aside className="lg:col-span-4 space-y-10">
               ...
               <div className="space-y-4">
                 {category?.calculators
                   ...
                   .map(related => (
                     <Link 
                       key={related.slug} 
                       href={`/sports/${category.id}/${related.slug}`}
                       className="flex items-center justify-between p-4 rounded-2xl hover:bg-emerald-50 transition-colors group"
                     >
                       <span className="font-bold text-gray-700 group-hover:text-emerald-600 transition-colors">{related.name}</span>
            ...
            <div className="bg-[#111] p-8 rounded-[2rem] text-white overflow-hidden relative">
               <div className="relative z-10">
                 ...
                 <p className="text-gray-400 mb-8 font-medium">Download our mobile app for offline access to all sports tools.</p>
